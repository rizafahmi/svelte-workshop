...
```shell
npm create vite@latest # pilih svelte
cd ngobrolin_web
npm install
npm run dev
```
...
Sebelumnya kita sudah melihat bagaimana cara mem-binding variable di Svelte, dengan notasi `{}`.
...
> Namun, kok jumlah kata tidak update?
...
```diff
- <main class={isDarkMode === true ? 'dark' : 'light'}>
+ <main class:dark={isDarkMode === true}>
```
